...
If the \gls{orm} calculation includes the radiation loss and the energy gain by the \gls{rf} cavity, the orbit length is fixed by the phase stability principle and the energy shifts contributions in the \gls{orm} is automatically included\footnote{The calculations performed with transverse coordinates are typically called 4D simulations and when it includes the transverse and longitudinal coordinates it is called 6D simulations.}.
...
Let $\vec{K} = \left(K_1, \ldots, K_{\mathrm{N}_{\mathrm{quads}}}\right)$ be the vector that represents the quadrupoles parameters. The constraint is implemented by extending the residue vector and the LOCO jacobian matrix:
\begin{align}
...
Therefore, the $\Delta K$ constraints can be interpreted as modifications to the GN method that are similar to the adjusts made in the LM method. The main difference is that the constraints include a gradient-descent component in the parameters $\Delta K$, while the LM method inclues that component in all directions given by the columns of $\mathbf{J}$. The importance of this constraint component to the fitting is controlled by the magnitude of $\mathbf{J}_{\Delta K}$, i.e., by the constraint weights $w_q$ and the normalization factor $\sigma_{\Delta K}$.

From equation~\eqref{eq:gn_modified}, it can be seen that the constraints can be implemented by keeping the jacobian matrix $\mathbf{J}$ unchanged, but adding only the contribution from the independent block-diagonal matrix $\mathbf{J}_{\Delta K}$ to the parameters calculations. In that way, the LM method with constraints solves the following problem
\begin{equation}
