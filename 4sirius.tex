...
Considering that an origin for the coordinate $s$ is defined, let's assume that the BPM longitudinal position is smaller than the corrector (the BPM is behind the corretor), the general transfer matrix between these two elements is
\begin{equation}
    \begin{bmatrix}
    \Delta x \\
    \Delta x' \\
...
% In adjusted BPMs gains and roll angles it can be clearly identified that the values for 6\ts{th} and 7\ts{th} stand out compared the remaining BPMs, even reaching negative horizontal gains. CH and CV gains near the region where the swap was performed also indicate that this region is problematic. These obvious outliers, specially the unrealistic horizontal gains, can be used to point out that there is a type of o problem in 3\ts{rd} and 2\ts{nd} BPMs that can not be explained with transformations of scales and rotations. 
